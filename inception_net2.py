...
from blocks import inception_block, upsampling_block
...
def run_model():
    ...
    tag = 'inc_model_b5_relu_w10'
    model_dir += '/{0}'.format(tag)
    
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)
    
    model = model1(tag, (IMG_HEIGHT, IMG_WIDTH, NUM_CHANNELS, OUTPUT_CHANNELS), 'relu', 10)
